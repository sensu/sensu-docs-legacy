...
1. Download Sensu from the [Sensu Downloads][1] page, or by using the `wget`
   ...
   ~~~ shell
   wget https://sensu.global.ssl.fastly.net/solaris/pkg/sensu-0.25.3-1.i386.pkg
   ~~~

   Alternatively, for SPARC packages:

   ~~~ shell
   wget https://sensu.global.ssl.fastly.net/solaris/pkg/sensu-0.25.3-1.sparc.pkg
   ~~~

2. Install the `sensu-0.25.3-1.i386.pkg` package using the `pkgadd` utility:

   ~~~ shell
   $ su
   $ pkgadd -d sensu-0.25.3-1.i386.pkg
   ~~~
...
1. Download Sensu from the [Sensu Downloads][1] page, or by using the `wget`
   ...
   ~~~ shell
   wget https://sensu.global.ssl.fastly.net/solaris/ips/sensu-0.25.3-1.p5p
   ~~~

2. Install the `sensu-0.25.3-1.p5p` package using the `pkg` utility:

   ~~~ shell
   $ sudo pkg install -g sensu-0.25.3-1.p5p developer/versioning/sensu
   ~~~

3. Download and run the Sensu [post-install script][12]:

   ~~~ shell
   $ wget https://sensuapp.org/docs/0.25/files/postinst.sh
   $ chmod +x postinst.sh
   $ sudo ./postinst.sh
   ~~~
...
[12]: /docs/0.25/files/postinst.sh
