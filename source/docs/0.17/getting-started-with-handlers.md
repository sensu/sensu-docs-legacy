...
_NOTE: the following Ruby installation steps may differ depending on your platform. You may have already done the following steps as part of the [getting started with checks guide](getting-started-with-checks)._
...
The following is an example Sensu handler definition, a JSON configuration file located at `/etc/sensu/conf.d/event_file.json`. This handler definition uses the [event-file plugin](#install-dependencies) to write event data to a file. The handler is named `file` and it runs `/etc/sensu/plugins/event-file.rb`, providing it with JSON event data via `STDIN`.
...
TCP and UDP event handlers send the event data to a remote socket. Both handler types use the same definition attributes and the following TCP handler instructions can work for UDP with minor adjustments.
...
The following command will create a TCP socket server listening on port 6000, run it in a separate terminal window.

_NOTE: netcat will output messages that it receives in the terminal window._
...
Transport handlers publish event data to the Sensu transport (message bus). Transport event handlers are used to deliver event data to other services, using the Sensu transport.
