...
1. Download the Sensu Redis Sentinel configuraiton file.
   ...
   ~~~ shell
   sudo wget -O /etc/redis/sentinel.conf http://sensuapp.org/docs/0.25/files/sentinel.conf
   ~~~
...
6. The Redis package does not provide an init script for Sentinel. Run the
   ...
   ~~~ shell
   sudo wget -O /etc/init.d/redis-sentinel http://sensuapp.org/docs/0.25/files/redis-sentinel
   ~~~
