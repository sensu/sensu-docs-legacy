...
2. Install the `sensu-0.23.2-1.i386.pkg` package using the `pkgadd` utility:
   ...
   ~~~ shell
   $ su
   $ pkgadd -d sensu-0.23.2-1.i386.pkg
   ~~~
...
2. Install the `sensu-0.23.2-2.p5p` package using the `pkg` utility:
   ...
   ~~~ shell
   $ sudo pkg install -g sensu-0.23.2-2.p5p developer/versioning/sensu
   ~~~

3. Download and run the Sensu [post-install script][12]:

   ~~~ shell
   $ wget https://sensuapp.org/docs/0.23/files/postinst.sh
   $ chmod +x postinst.sh
   $ sudo ./postinst.sh
   ~~~
...
[12]: /docs/0.23/files/postinst.sh
